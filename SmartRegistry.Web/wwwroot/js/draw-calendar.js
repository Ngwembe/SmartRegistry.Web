...
function filterSchedulesChart(subjectId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var beginFilterDate = $("#FilterFromDate").val();
            var endFilterDate = $("#FilterToDate").val();

            console.log({ beginFilterDate });
            console.log({ endFilterDate });

            for (var index = 0; index < response.scheduleData.length; index++) {
                if (moment(response.scheduleData[index].date).isBetween(beginFilterDate, endFilterDate, undefined, '[]')) {
                    filteredSchedules.push(response.scheduleData[index]);
                }
            }

            console.log({ filteredSchedules });

            response.scheduleData = filteredSchedules;
...
function populateChart(data) {
    ...
    //  Initialize Chart (0,0)
    //var currentSemester = new Date().getMonth();

    //if (currentSemester <= 6) {
    //    absent.push(moment(new Date(1, 1, new Date().getFullYear()), "dd-mm-yyyy"));
    //    present.push(moment(new Date(1, 1, new Date().getFullYear()), "dd-mm-yyyy"));
    //}
    //else {
    //    absent.push(moment(Date.UTC(1, 7, new Date().getFullYear()), "dd-mm-yyyy"));
    //    present.push(moment(Date.UTC(1, 7, new Date().getFullYear()), "dd-mm-yyyy"));
    //}

    //present.push(0);
    //absent.push(0);

    for (const schedule of data.scheduleData) {
    ...
    var chart = new Highcharts.Chart({
        ...
        title: { text: `Attendance for ${data.subject.name} (${data.subject.code}) ${new Date().getFullYear()}` },
        credits: { enabled: false },
        //xAxis: {
        //    type: 'datetime',
        //    min: currentSemester <= 6 ? moment(Date.UTC(1, 1, new Date().getFullYear()), "dd-mm-yyyy") : moment(Date.UTC(1, 7, new Date().getFullYear()), "dd-mm-yyyy"),
        //    max: moment(Date.UTC(new Date())),
